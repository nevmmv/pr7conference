...
public class SetAnnouncementServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
    	...
    	Query<Conference> query = OfyService.ofy().load()
				.type(Conference.class).filter("maxAttendees <", 5).order("maxAttendees");
        ...
        if (conferenceNames.size() > 0) {
            ...
            OfyService.putMemcache(Constants.MEMCACHE_ANNOUNCEMENTS_KEY,
            		new Announcement(announcementStringBuilder.toString()));
