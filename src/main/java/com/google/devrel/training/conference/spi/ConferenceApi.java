...
public class ConferenceApi {
    ...
    public Profile saveProfile(ProfileForm form, User user) throws UnauthorizedException {
        ...
        if (user == null) {
            throw new UnauthorizedException("Authorization required, ок да?");
        }
        ...
        if(form.getTeeShirtSize() != null) teeShirtSize = form.getTeeShirtSize();
        ...
        if(profile == null) {
        	profile = processNewProfile(userId, displayName, mainEmail, teeShirtSize);
        } else {
    ...
    private Profile processNewProfile(String userId, String displayName, String mainEmail, TeeShirtSize teeShirtSize) {
    ...
    public Profile getProfile(final User user) throws UnauthorizedException {
        ...
        Key<Profile> key = (Key<Profile>) Key.create(Profile.class,userId); // TODO
        Profile profile = OfyService.ofy().load().key(key).now(); // TODO load the Profile entity
        return profile;
    }
    
    @ApiMethod(name = "getConferencesCreated", path = "getConferencesCreated", httpMethod = HttpMethod.POST)
    List<Conference> getConferencesCreated(final User user) throws UnauthorizedException {
    	...
    	Query<Conference> query = ofy().load().type(Conference.class).
    			ancestor(user.getUserId()).order("name");
    ...
    public Conference createConference(final User user, final ConferenceForm conferenceForm)
        ...
        // Save Conference and Profile Entities
         ...
         queue.add(
        		 TaskOptions.Builder.withUrl("/crons/send_confirmation_email").
    ...
    public WrappedBoolean registerForConference(final User user,
        ...
        WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>(){
        	...
        	public WrappedBoolean run() {
                ...
                Key<Profile> key = (Key<Profile>) Key.create(Profile.class,userId); // TODO
                Profile profile = OfyService.ofy().load().key(key).now();
    ...
    public Collection<Conference> getConferencesToAttend(final User user)
        ...
        Key<Profile> key = (Key<Profile>) Key.create(Profile.class, user.getUserId()); // TODO
        Profile profile = OfyService.ofy().load().key(key).now(); // Change this;
        if (profile == null) {
        ...
        List<Key<Conference>> confrencesKeyList = new ArrayList<>(0);
        
        for(String s_key : keyStringsToAttend) {
        	confrencesKeyList.add(Key.create(Conference.class, s_key));
        }
		
        return ofy().load().keys(confrencesKeyList).values();  // change this
    }
    ...
    */   
	...
	public WrappedBoolean unregisterFromConference(final User user,
        ...
        WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>(){
        	...
        	public WrappedBoolean run() {
                ...
                if (!profile.getConferenceKeysToAttend().contains(
                        websafeConferenceKey)) {
                    return new WrappedBoolean (false, "Not yet registered");
                } else {
        ...
        if (!result.getResult()) {
            ...
            else if (result.getReason() == "Not yet registered") {
                throw new ConflictException("You are not registered yet");
            }
            else {
                throw new ForbiddenException("Unknown exception");
            }
	...
	public Announcement getAnnouncement() {
		return (Announcement) 
				OfyService.getMemcache(Constants.MEMCACHE_ANNOUNCEMENTS_KEY);
	}
